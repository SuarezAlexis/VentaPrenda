...
namespace VentaPrenda.DAO.Concrete
{
    public class NotaDaoMySQL : INotaDao
    {
        ...
        private static readonly string LIST_SELECT_SQL = "SELECT N.ID, C.Nombre Cliente, CASE WHEN Estatus = 0 THEN 'Cancelado' WHEN Estatus = 1 THEN 'Pendiente' WHEN Estatus = 2 THEN 'Terminado' WHEN Estatus = 3 THEN 'Entregado' WHEN Estatus = 4 THEN 'Caducado' ELSE '' END Estatus, Recibido, Entregado FechaEntrega, D.Nombre Descuento, R.Nombre Atendio FROM Nota N JOIN Cliente C ON (C.ID = N.Cliente) LEFT JOIN Descuento D ON (D.ID = N.Descuento) LEFT JOIN DatosHistorial DH ON(DH.Valor = N.ID AND DH.Tabla = 'Nota' AND DH.Columna = 'ID' AND Operacion = 'I') LEFT JOIN Historial H ON(H.ID = DH.Historial) LEFT JOIN Usuario R ON(R.ID = H.Usuario)";
