...
namespace VentaPrenda.DAO.Concrete
{
    ...
    {
        private static readonly string SELECT_SQL = "SELECT N.ID NotaID, N.Estatus, N.Cliente ClienteID, C.Nombre ClienteNombre, C.Domicilio, C.Colonia, C.CP, C.Telefono, C.Email, C.Habilitado ClienteHabilitado, Recibido, Entregado, Observaciones, N.Descuento DescuentoID, D.Nombre DescuentoNombre, D.VigenciaInicio, D.VigenciaFin, D.MontoMinimo, D.CantMinima, D.Porcentaje, D.Unidades, D.SoloNota, PI.ID PrendaItemID, PI.Cantidad PrendaItemCantidad, PI.Prenda PrendaID, P.Nombre PrendaNombre, P.Habilitado PrendaHabilitado, PI.Tipo TipoID, T.Nombre TipoNombre, T.Habilitado TipoHabilitado, PI.Color ColorID, Co.Nombre ColorNombre, Co.Habilitado ColorHabilitado, SI.ID ServicioItemID, SI.Cantidad ServicioItemCantidad, SI.Servicio ServicioID, S.Nombre ServicioNombre, S.Descripcion ServicioDescripcion, S.Costo ServicioCosto, S.Habilitado ServicioHabilitado, SI.Monto ServicioItemMonto, SI.Descuento ServicioItemDescuento, D2.Nombre SIDescNombre, D2.VigenciaInicio SIDescVigenciaInicio, D2.VigenciaFin SIDescVigenciaFin, D2.MontoMinimo SIDescMontoMinimo, D2.CantMinima SIDescCantMinima, D2.Porcentaje SIDescPorcentaje, D2.Unidades SIDescUnidades, D2.SoloNota SIDescSoloNota, SI.Encargado EncargadoID, E.Nombre EncargadoNombre, E.Username EncargadoUsername FROM Nota N JOIN Cliente C ON(C.ID = N.Cliente) LEFT JOIN Descuento D ON(D.ID = N.Descuento) JOIN PrendaItem PI ON(PI.Nota = N.ID) JOIN Prenda P ON(P.ID = PI.Prenda) LEFT JOIN TipoPrenda T ON(T.ID = PI.Tipo) JOIN Color Co ON(Co.ID = PI.Color) JOIN ServicioItem SI ON(SI.PrendaItem = PI.ID) JOIN Servicio S ON(S.ID = SI.Servicio) LEFT JOIN Descuento D2 ON(D2.ID = SI.Descuento) LEFT JOIN Usuario E ON(E.ID = SI.Encargado)";
        private static readonly string LIST_SELECT_SQL = "SELECT N.ID, C.Nombre Cliente, CASE WHEN Estatus = 0 THEN 'Pendiente' WHEN Estatus = 1 THEN 'Terminado' WHEN Estatus = 2 THEN 'Entregado' WHEN Estatus = 3 THEN 'Caducado' ELSE '' END Estatus, Recibido, Entregado FechaEntrega, D.Nombre Descuento FROM Nota N JOIN Cliente C ON (C.ID = N.Cliente) LEFT JOIN Descuento D ON (D.ID = N.Descuento)";
        ...
        private void GuardarPrendas(List<PrendaItemDto> Prendas)
        {
            ...
            foreach (PrendaItemDto p in Prendas)
            {
                if (p.ID < 0)
                    ...
                    update_query += "UPDATE PrendaItem SET Cantidad = " + p.Cantidad + ", Prenda = " + p.Prenda.ID + ", Tipo = " + (p.TipoPrenda != null ? p.TipoPrenda.ID.ToString() : "NULL") + ", Color = " + p.Color.ID + " WHERE ID = " + p.ID + "; ";
                id_list += p.ID >= 0? (p.ID + ",") : "";
            }
